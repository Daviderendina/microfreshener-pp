...
from k8s_template.kobject_generators import MF_NODEPORT_SERVICE_SUFFIX, MF_CLUSTERIP_SERVICE_SUFFIX
...
class TestRefactoringAddMessageRouter(TestCase):
    ...
    def test_add_service_with_pod_resource(self):
        ...
        # Check name
        service_name = f"{k_pod_3.name}-{MF_CLUSTERIP_SERVICE_SUFFIX}"
    ...
    def test_add_service_with_deploy_resource(self):
        ...
        # Check name
        service_name = f"{k_deploy.name}-{MF_CLUSTERIP_SERVICE_SUFFIX}"
    ...
    def test_add_service_with_pod_resource_and_existing_service(self):
        ...
        self.assertEqual(len([n for n in model.nodes]), 6)
        self.assertTrue(k_service in cluster.services)
