...
class TestAddAPIGatewayRefactoring(TestCase):
    ...
    def test_pod_with_host_port(self):
        ...
        # Cluster
        k_pod = KPod.from_dict(POD_WITH_ONE_CONTAINER)
        k_pod.get_containers()[0].ports[0]["hostPort"] = 90
        cluster.add_object(k_pod, KObjectKind.POD)
        ...
        container_name = k_pod.get_containers()[0].name
        k_pod_port_strings = [
            f"{container_name}.{p.get('name', k_pod.get_fullname()+'-port-'+str(p['containerPort'])+'-MF')} {p.get('protocol', 'PROTOCOL?')} {p['containerPort']} {p['hostPort']}"
            for p in k_pod.get_containers()[0].ports if p.get("hostPort")]
        k_svc_port_strings = [
            f"{p.get('name', '')} {p.get('protocol', 'PROTOCOL?')} {p['port']} {p['node_port']}"
            for p in k_service.get_ports()]
    ...
    def test_deployment_with_host_port(self):
        ...
        # Cluster
        k_deploy = KDeployment.from_dict(DEPLOYMENT_WITH_ONE_CONTAINER)
        k_deploy.get_pod_template_spec().get_containers()[0].ports[0]["hostPort"] = 90
        cluster.add_object(k_deploy, KObjectKind.DEPLOYMENT)
        ...
        k_pod_port_strings = [
            f"{p.get('name', k_deploy.get_fullname()+'-port-'+str(p['containerPort'])+'-MF')} {p.get('protocol', 'PROTOCOL?')} {p['containerPort']} {p['hostPort']}"
            for p in k_deploy.get_containers()[0].ports if p.get("hostPort")]
        k_svc_port_strings = [
            f"{p.get('name', '')} {p.get('protocol', 'PROTOCOL?')} {p['port']} {p['node_port']}"
            for p in k_service.get_ports()]
    ...
    def test_pod_with_two_container_host_port(self):
        ...
        k_pod.get_containers()[0].ports[0]['hostPort'] = 80
        k_pod.get_containers()[1].ports[0]['hostPort'] = 81
        cluster.add_object(k_pod, KObjectKind.POD)
        ...
        k_pod_port_strings = [
            f"{p.get('name', k_pod.get_containers()[0].name+'.'+k_pod.get_fullname()+'-port-'+str(p['containerPort'])+'-MF')} {p.get('protocol', 'PROTOCOL?')} {p['containerPort']} {p.get('hostPort', p['containerPort'])}"
            for p in k_pod.get_containers()[0].ports]
        k_pod_port_strings += [
            f"{p.get('name', k_pod.get_containers()[1].name+'.'+k_pod.get_fullname()+'-port-'+str(p['containerPort'])+'-MF')} {p.get('protocol', 'PROTOCOL?')} {p['containerPort']} {p.get('hostPort', p['containerPort'])}"
    ...
    def test_pod_with_two_container_host_port_equal_ports(self):
        model = None
        cluster = None
        model = MicroToscaModel("model")
        model.add_group(Edge("edge"))
